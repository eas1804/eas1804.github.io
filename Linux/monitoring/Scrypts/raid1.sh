...
# Проверяем вывод команды /usr/sbin/mdadm -D /dev/md0 на наличие строки "removed"
if /usr/sbin/mdadm -D /dev/md0 | grep -q "removed"; then
...
# Проверяем вывод команды /usr/sbin/mdadm -D /dev/md1 на наличие строки "removed"
if /usr/sbin/mdadm -D /dev/md1 | grep -q "removed"; then
...
if /usr/sbin/mdadm -D /dev/md2 | grep -q "removed"; then
...
if /usr/sbin/mdadm -D /dev/md3 | grep -q "removed"; then
