...
USER_NAME=$1
NO_ROOT_FOLDER=/home/exch_adm/OpenVPN_Client/

ls $KEY_FOLDER || mkdir -p $KEY_FOLDER
ls /etc/openvpn/ccd || mkdir /etc/openvpn/ccd
ls $NO_ROOT_FOLDER || mkdir -p $NO_ROOT_FOLDER
...
##### Part 2. Create Opne File
cd $KEY_FOLDER/$USER_NAME

KEY_DIR=$KEY_FOLDER/$USER_NAME
OUTPUT_DIR=$KEY_FOLDER/$USER_NAME
BASE_CONFIG=./$USER_NAME.ovpn
